...
def build_vgg19_model(input_shape=(32, 32, 3), num_classes=100):
        weight_decay = 1e-4  # 调整此值以控制正则化强度

        model = models.Sequential()

        # Conv1
        model.add(layers.Conv2D(64, (3, 3), activation='relu', padding='same', input_shape=input_shape,
                                kernel_initializer=tf.keras.initializers.he_normal(), kernel_regularizer=regularizers.l2(weight_decay)))
...
model = build_vgg19_model(input_shape=(32, 32, 3), num_classes=100)
...
optimizer = optimizers.SGD(learning_rate=learning_rate, momentum=0.9, decay=1e-6, nesterov=True, clipvalue=0.5)  # clipvalue避免梯度爆炸
